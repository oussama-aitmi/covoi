jQuery(document).ready(function($){
	...
	var $advertFilter 			= $('[name="advertfilter"]');
	var $userFilter 			= $('[name="userfilter"]');
	var $conLoading 			= $('.con-loading');
		...
		if( conAjaxing == false ){
			...
			$.ajax({
				...
				success: function(response){
					...
					$('.ajax-conversations').html( response.conversations );
					$('.ajax-conversations-pagination').html( response.pagination );
					if( callback ){
						callback();
					}
				},
	...
	* On toggle change
	*/
	$(document).on('change', '#toggle-list', function(){
		$('.conversations-list input').prop('checked', $(this).prop('checked') )
	});
	...
	*/
	$(document).on('click', '.all-conversations', function(e){
		e.preventDefault();
		$conForm.find('input[type="text"]').val('');
		$conForm.submit();
	});
	...
	*/
    $conForm.on('submit', function(e){
    	e.preventDefault();
    	var fields = $conForm.serializeArray();
	    $.each( fields, function( i, field ) {
	      conData[field.name] = field.value;
	    });
	    conData.page = 1;
    	fetchConversations();
    });
	...
	*/
	function fetchMesages( con_id ){
		...
		if( con_id ){
			$messLoading.removeClass('hidden');
			$.ajax({
				url: js_data.ajaxurl,
				method: 'POST',
				data: {
	...
	*/
	function updateUnreadCount( unread ){
      ...
      /* By Ous
      var unread = $('.special-nav .unread-badge').text();

      var unreadMessage = parseInt(unread) - parseInt(sidebarCount);
      console.log('unreadMessage : '+ unreadMessage);
      console.log('sidebarCount : '+ sidebarCount);

      unreadMessage = Math.abs(unreadMessage);
      if(unreadMessage>0){
        count = parseInt( count - unreadMessage);
        console.log('count : '+ count);
      }
      if(typeof unread == 'undefined' || unread=='undefined' || unread=="" || unread=="NaN" ){
        count = $('.author-sidebar .user-requesting').text();
        console.log('undefined');
      }

      var userRequesting = $('.author-sidebar .user-requesting').text();
      var messageUnread = $('.author-sidebar .unread-badge').text();

      if(typeof userRequesting == 'undefined' || userRequesting=='undefined' || userRequesting=="" || userRequesting=="NaN" ){
        userRequesting =0;
      }
      if(typeof messageUnread == 'undefined' || messageUnread=='undefined' || messageUnread=="" || messageUnread=="NaN" ){
        messageUnread =0;
      }
      var notification = parseInt(userRequesting) + parseInt(messageUnread);


      $('.special-nav .unread-badge').remove();
      $('.special-nav .messages-unread-count').append( '<span class="unread-badge">'+notification+'</span>' );
      */
	...
	*/
	$(document).on('click', '.start-messages', function(e){
		...
		$('.paste-advert-title').html($parent.find('.message-advert-title').html());
		...
		if( isMobile ){
			$("html, body").stop().animate(
				{
					scrollTop: $('.messages-box').offset().top
				},
				{
					duration: 1200
				}
	...
	function sendMessage( $this ){
		...
		if( $this.find('textarea').val() !== '' ){
			$messLoading.removeClass('hidden');
      $this.find('textarea').prop("disabled", true).css('background-color', '#f7f7f7');
    		$.ajax({
    			url: js_data.ajaxurl,
    			method: 'POST',
    			data: formData,
    			...
    			complete: function(){
    				$messLoading.addClass('hidden');
            $this.find('textarea').prop("disabled", false).css('background-color', '');
    			}
    		});
    	}
	}
	...
	*/
	$(document).on('keypress', 'form.message-form', function(e){
    	if( (e.which == 13 || e.keyCode == 13 ) && !e.shiftKey && !isMobile ) {
    		e.preventDefault();
    		sendMessage( $(this) );
    	}
    });
	...
	/*
	* switch textarea placeholder for mobile phones
	*/
	var $messagesTextarea = $('.messages-textarea');
	if( isMobile && $messagesTextarea.length > 0 ){
		$messagesTextarea.attr('placeholder', $messagesTextarea.data('mobileplace'));
	}
